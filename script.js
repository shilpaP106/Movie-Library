  ...
  function searchBoth(title, genre) {
    ...
    if (title.trim() !== '' || genre.trim() !== '') {
      results = movies.filter(movie => (
        (title.trim() === '' || movie.title.toLowerCase().includes(title.toLowerCase().trim())) &&
        (genre.trim() === '' || movie.genre.toLowerCase().includes(genre.toLowerCase().trim()))
      ));
    }
  
    if (results.length === 0) {
      resultDisplay.innerHTML = 'No results found for your search.';
    } else {
      resultDisplay.innerHTML = '';
      results.forEach(movie => {
        resultDisplay.innerHTML += `<p>${movie.title} - ${movie.genre}</p>`;
      });
    }
  }
